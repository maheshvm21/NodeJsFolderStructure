...
function Controller(objCollection) {
    ...
    app.post('/' + global.config.version + '/get/all/users', async function (req, res) {
        let [err, result] = await Services.getUsers(req.body)
        if (!err) {
            res.json(responseWrapper.getResponse({}, result, 200, req.body));
        } else {
    ...
    app.post('/' + global.config.version + '/pam/get/all/users', async function (req, res) {
